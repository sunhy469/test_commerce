...
SIMPLE_CHAT_SYSTEM = """你是一个简洁、友好的中文AI助手。
要求：
1) 直接回答用户问题，不触发外部业务流程。
2) 不要引导到1688、供应链、排行榜等系统动作。
3) 回答简短清晰，必要时分点说明。
"""
...
async def chat(req: ChatRequest):
    """处理用户对话消息：当前采用简单对话模式，不走多智能体自动化流程"""
    session_id = req.session_id or str(uuid.uuid4())[:8]
    user_message = (req.message or "").strip()
    _save_chat(session_id, "user", user_message)

    reply = await _generate_simple_reply(user_message)
    result_data = {"type": "text", "content": reply}
    action = "general_chat"

    _save_chat(session_id, "assistant", reply, action, json.dumps(result_data, ensure_ascii=False, default=str))
    store.log_activity("chat", "AI对话: general_chat", user_message[:80])
...
async def _generate_simple_reply(message: str) -> str:
    if not message:
        return "你好，我在。你可以直接告诉我你想聊什么。"

    try:
        raw = await claude.analyze(SIMPLE_CHAT_SYSTEM, message)
        parsed = None
        try:
        ...
        return text or "我收到了，你可以继续说具体一点。"
    except Exception:
        return "我收到了。当前先按简单对话模式回复，你可以继续提问。"
