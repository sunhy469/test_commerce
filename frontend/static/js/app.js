...
function renderDashboardPage(data) {
    ...
    return `${renderFilterBar()}<section class="section-card p-5"><div class="flex justify-between"><div><h2 class="serif-title text-3xl font-black">Seller Center Dashboard</h2><p class="text-sm text-[var(--muted)] mt-1">经营、履约、财务、内容一体化总览</p></div>${data.state==='mock'?'<span class="status-tag warn">Mock Fallback</span>':''}</div><div class="seller-grid-kpi mt-4">${kpi}</div></section><div class="grid xl:grid-cols-3 gap-4"><section class="section-card p-5 xl:col-span-2"><h3 class="font-bold mb-3">GMV / 订单 / 买家趋势</h3><canvas id="chartTrend" height="120"></canvas></section><section class="section-card p-5"><h3 class="font-bold mb-3">收入构成</h3><canvas id="chartRevenueMix" height="160"></canvas></section></div><div class="grid xl:grid-cols-3 gap-4"><section class="section-card p-5"><h3 class="font-bold mb-3">流量来源</h3><canvas id="chartTraffic" height="140"></canvas></section><section class="section-card p-5"><h3 class="font-bold mb-3">订单状态分布</h3><canvas id="chartOrderStatus" height="140"></canvas></section><section class="section-card p-5"><h3 class="font-bold mb-3">店铺健康度</h3><div class="text-4xl font-black">${data.health.score}</div><div class="text-xs text-[var(--muted)] mt-2">违规率低于行业均值 ${data.health.delta}%</div></section></div><div class="grid xl:grid-cols-2 gap-4">${sectionCard('热销商品 Top10','GMV 排行',topProducts)}${sectionCard('待办与预警','订单、退款、库存、违规',todos)}</div><section class="section-card p-5"><div class="flex flex-wrap gap-2 mb-4">${['overview','orders','products','contentlive','affiliate','logistics','finance','health'].map(m=>`<button class="chip ${dashboardModule===m?'on':''}" onclick="switchDashboardModule('${m}')">${({overview:'总览扩展',orders:'订单管理',products:'商品管理',contentlive:'内容直播',affiliate:'联盟带货',logistics:'物流履约',finance:'财务结算',health:'店铺健康'})[m]}</button>`).join('')}</div><div id="dashboardModuleRoot"></div></section>`;
}
...
function loadOrdersPage() {
    renderSimpleModule('dashboardModuleRoot', '订单管理 Orders', '多条件筛选、状态标签、异常面板（已保留原采购接口）',
...
function loadProductsPage() {
    renderSimpleModule('dashboardModuleRoot', '商品管理 Products', '商品总览、商品列表、库存管理、质量建议',
...
function loadContentLivePage() { renderSimpleModule('dashboardModuleRoot', '内容与直播 Content/LIVE', '短视频与直播带货分析', [{label:'视频带货GMV',value:'$46,920'},{label:'视频CTR',value:'3.9%'},{label:'直播场次',value:'24'},{label:'直播GMV',value:'$32,840'}], ['维度','指标','值','备注'], [['短视频','CTOR','2.4%','周同比+0.2%'],['直播','平均停留时长','2m 18s','高于类目均值']]); }
function loadAffiliatePage() { renderSimpleModule('dashboardModuleRoot', '联盟带货 Affiliate', '达人 / 商品 / 视频 / 直播多维分析', [{label:'达人数',value:'86'},{label:'达人GMV',value:'$39,200'},{label:'达人佣金',value:'$6,820'},{label:'合作中',value:'53'}], ['达人','带货商品','GMV','佣金','状态'], [['@luna_live','17','$8,420','$1,320',statusTag('success','合作中')],['@techdaisy','8','$4,780','$690',statusTag('info','观察')]]); }
function loadLogisticsPage() { renderSimpleModule('dashboardModuleRoot', '物流履约 Logistics', 'SLA 风险、仓库表现、物流异常', [{label:'On-time Delivery',value:'96.2%'},{label:'3-Day Delivery',value:'91.4%'},{label:'Late Dispatch',value:'2.1%'},{label:'Valid Tracking',value:'99.1%'}], ['仓库','包裹量','准时率','异常数','风险'], [['US-WH1','8,420','97.1%','42',statusTag('success','低')],['SEA-WH2','5,180','93.8%','76',statusTag('warn','中')]]); }
function loadFinancePage() { renderSimpleModule('dashboardModuleRoot', '财务结算 Finance', '收入、结算、退款与可提现管理', [{label:'On Hold',value:'$4,820'},{label:'Processing',value:'$12,430'},{label:'Paid',value:'$108,420'},{label:'可提现',value:'$76,280'}], ['结算单号','周期','应结算','平台费用','状态'], [['ST202604-1','04/01-04/07','$23,400','$1,120',statusTag('success','已打款')],['ST202604-2','04/08-04/14','$26,800','$1,340',statusTag('info','处理中')]]); }
function loadHealthPage() { renderSimpleModule('dashboardModuleRoot', '店铺健康 Shop Health', '评分 + 指标诊断 + 违规记录 + Top Opportunity', [{label:'健康分',value:'92'},{label:'商品满意度',value:'4.7/5'},{label:'客服体验',value:'95%'},{label:'违规记录',value:'2'}], ['模块','当前值','阈值','建议'], [['物流履约','93.8%','>=95%',statusTag('warn','提升海外仓备货')],['退款率','1.6%','<=2.0%',statusTag('success','保持')]]); }
function loadMessagesPage() { renderSimpleModule('dashboardModuleRoot', '客户消息 Messages', '售前咨询、售后工单与SLA', [{label:'未读会话',value:'28'},{label:'待处理售后',value:'13'},{label:'平均响应',value:'4m 12s'},{label:'满意度',value:'95%'}], ['会话ID','用户','类型','状态','更新时间'], [['MSG-1021','Olivia','售前咨询',statusTag('info','处理中'),'10:21'],['MSG-1022','Lucas','退款咨询',statusTag('warn','待响应'),'10:08']]); }
function loadSettingsPage() { renderSimpleModule('dashboardModuleRoot', '设置 Settings', '账号权限、偏好与自动化规则', [{label:'角色数',value:'8'},{label:'已启用自动化',value:'12'},{label:'API Token',value:'3'},{label:'告警规则',value:'16'}], ['配置项','值','状态'], [['默认站点','US',statusTag('success','启用')],['风险告警','订单异常/违规/低库存',statusTag('success','启用')]]); }
